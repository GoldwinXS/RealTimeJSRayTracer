...
export class TextureAtlasBuilder {
  ...
  packVoxelGeometries(voxelGeometries) {
    ...
    let sortedGeometries = this.sortGeometries(geometriesArray);

    // Assign atlas positions to each voxel geometry.
    this.assignAtlasPositions(sortedGeometries);
  }

  assignAtlasPositions(sortedGeometries) {
    try {
      ...
      let rowHeight = 0,
        layerDepth = 0;
      const marginSize = 1;
      sortedGeometries.forEach((geometry) => {
        geometry.textureAtlasSize = this.textureAtlasSize;
        // Check if we have assigned this geometry before
        ...
        if (
          ...
          console.log("Problem adding normalized positions");
        }
  ...
  sortGeometries(geometriesArray) {
    return geometriesArray.sort((a, b) => {
      const volumeA =
